...
export const createWorkstation = async (req: Request, res: Response) => {
  try {
    const { unitId, mobo, cpu, ram, gpu, psu, storage, monitor, webcam, headset, keyboard, lanCable, cableAdaptor, wifiAdaptor, mouse, ups, monitors, status, assignedUserIds, notes, deployedItemIds } = req.body;
    ...
    const workstation = await prisma.$transaction(async (tx) => {
      const ws = await tx.workstation.create({
        data: {
          unitId, mobo, cpu, ram, gpu, psu, storage, monitor, webcam, headset, keyboard, lanCable, cableAdaptor, wifiAdaptor, mouse, ups, status, notes,
          assignedUsers: {
...
export const updateWorkstation = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const { unitId, mobo, cpu, ram, gpu, psu, storage, monitor, webcam, headset, keyboard, lanCable, cableAdaptor, wifiAdaptor, mouse, ups, monitors, status, assignedUserIds, notes, deployedItemIds, releasedItemIds } = req.body;
