...
const getGsapAssets = async (req, res) => {
    if (!PRIVATE_KEY || !CLIENT_EMAIL || !SPREADSHEET_ID)
        return res.status(500).json({ error: 'Server config error' });
    try {
        const jwt = new google_auth_library_1.JWT({ email: CLIENT_EMAIL, key: PRIVATE_KEY.replace(/\\n/g, '\n'), scopes: ['https://www.googleapis.com/auth/spreadsheets'] });
        const doc = new google_spreadsheet_1.GoogleSpreadsheet(SPREADSHEET_ID, jwt);
        await doc.loadInfo();
        let sheet = doc.sheetsById[SHEET_ID];
        if (!sheet) {
            sheet = await doc.addSheet({ sheetId: SHEET_ID, title: 'GSAP Library' });
            await sheet.setHeaderRow(['id', 'title', 'description', 'documentationUrl', 'codeSnippet', 'variables', 'embedVideo', 'likes', 'image', 'dateAdded']);
        }
        // --- GET ---
        if (req.method === 'GET') {
        ...
        // --- PUT (Update / Like) ---
            ...
            if (action === 'update') {
                ...
                if (updates.title)
                    row.set('title', updates.title);
                if (updates.description)
                    row.set('description', updates.description);
                if (updates.codeSnippet)
                    row.set('codeSnippet', updates.codeSnippet);
                if (updates.documentationUrl)
                    row.set('documentationUrl', updates.documentationUrl);
                if (updates.variables)
                    row.set('variables', updates.variables);
                if (updates.embedVideo)
                    row.set('embedVideo', updates.embedVideo);
                if (updates.image)
                    row.set('image', updates.image);
                await row.save();
...
exports.getGsapAssets = getGsapAssets;
