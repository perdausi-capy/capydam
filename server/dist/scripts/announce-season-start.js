 ...
 */
const buildSeasonStartPayload = (seasonName) => {
    return {
        notify_all: true,
        comment: [
            {
...
const announceSeasonStart = async () => {
    try {
        ...
        if (token && chatId) {
            ...
            try {
                await axios_1.default.post(`https://api.clickup.com/api/v2/view/${chatId}/comment`, buildSeasonStartPayload(seasonName), { headers: { 'Authorization': token, 'Content-Type': 'application/json' } });
