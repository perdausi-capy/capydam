...
async function patchMetadata() {
    ...
    const oldDb = await promise_1.default.createConnection(oldDbConfig);
    ...
    for (const asset of assetsToFix) {
        ...
        if (!oldId)
            continue;
        try {
            const [rows] = await oldDb.execute(`SELECT n.resource_type_field, n.name AS value
                 FROM node n
                 JOIN resource_node rn ON n.ref = rn.node
                 WHERE rn.resource = ? 
                 AND n.resource_type_field IN (${placeholders})`, [oldId, ...ALL_IDS]);
            if (rows.length === 0)
                continue;
            let driveLink = null;
            let tags = [];
            ...
            for (const row of rows) {
                ...
                if (fieldId === FIELD_IDS.LINK)
                    driveLink = val;
                else if (fieldId === FIELD_IDS.TITLE)
                    valTitle = val;
                else if (fieldId === FIELD_IDS.DESC_CAPTION)
                    valCaption = val;
                else if (fieldId === FIELD_IDS.DESC_EXTENDED)
                    valExtended = val;
                else if (fieldId === FIELD_IDS.DESC_NOTES)
                    valNotes = val;
                else if (fieldId !== FIELD_IDS.FILENAME) {
                    if (val) {
                        const cleanParts = val.split(',').map((t) => t.trim()).filter((t) => t.length > 0);
