...
const supabase_1 = require("../utils/supabase"); // Adjust path to your supabase helper
    ...
    try {
        ...
        for (const asset of expiredAssets) {
            try {
                // Delete Main File
                if (asset.path)
                    await (0, supabase_1.deleteFromSupabase)(asset.path);
                // Delete Thumbnail
                if (asset.thumbnailPath)
                    await (0, supabase_1.deleteFromSupabase)(asset.thumbnailPath);
                // Delete Preview Frames (Video)
                if (asset.previewFrames && asset.previewFrames.length > 0) {
                    await Promise.all(asset.previewFrames.map(frame => (0, supabase_1.deleteFromSupabase)(frame)));
                }
            }
            catch (err) {
