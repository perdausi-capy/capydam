...
const launchDailyQuest = async () => {
    try {
        ...
        if (deactivated.count > 0)
            console.log(`   💀 Deactivated ${deactivated.count} old quest(s).`);
...
const notifyIntegrations = async (questionText) => {
    ...
    if (!token || !chatId)
        return;
    const clickUpPayload = buildClickUpPayload("DAILY QUEST NA THIS", "1f916", // 🤖 unicode
    "🤖", questionText);
    try {
        await axios_1.default.post(`https://api.clickup.com/api/v2/view/${chatId}/comment`, clickUpPayload, { headers: { 'Authorization': token, 'Content-Type': 'application/json' } });
    }
    catch (error) {
