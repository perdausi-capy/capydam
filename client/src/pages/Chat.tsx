...
// --- SKELETONS ---
const ChannelSkeleton = () => <div className="flex flex-col gap-2 px-2 animate-pulse">{[1, 2, 3].map(i => <div key={i} className="h-9 bg-gray-200 dark:bg-white/5 rounded-lg w-full" />)}</div>;
const MessageSkeleton = () => <div className="flex flex-col gap-6 px-6 py-4 animate-pulse">{[1, 2, 3].map(i => <div key={i} className="flex gap-4"><div className="w-10 h-10 rounded-full bg-gray-200 dark:bg-white/5 shrink-0" /><div className="flex flex-col gap-2 w-full"><div className="h-4 bg-gray-200 dark:bg-white/5 rounded w-24" /><div className="h-4 bg-gray-200 dark:bg-white/5 rounded w-full" /></div></div>)}</div>;
...
const Chat = () => {
  ...
  const parentMessage = messages.find(msg => msg.id === activeThread?.id);

  const publicChannels = channels.filter(c => c.type === 'channel');
  const privateGroups = channels.filter(c => c.type === 'group');
  const currentGroup = privateGroups.find(g => g.name === activeRoom);
  ...
  const handleAddMemberAction = (uid: string) => { 
      ...
      setTimeout(() => { 
          ...
          setIsAddingMember(false); // Close Modal
  ...
  // ✅ UPDATE: Open modal instead of using confirm()
  const handleDeleteChannelClick = (e: React.MouseEvent, id: string) => { 
      e.stopPropagation(); 
      setChannelToDeleteId(id);
  };

  // ✅ UPDATE: Actual delete function triggered by modal
  ...
  const handleDeleteMessage = (id: string) => setMessageToDelete(id);
  const confirmDeleteMessage = () => { if (messageToDelete) { deleteMessage(messageToDelete); setMessageToDelete(null); }};

  const handleStartEdit = (msg: Message) => { setEditingMessageId(msg.id); setEditText(msg.content); };
  const handleSaveEdit = () => { if(editingMessageId) editMessage(editingMessageId, editText); setEditingMessageId(null); };
  const handleKeyDownEdit = (e: React.KeyboardEvent) => { if(e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); handleSaveEdit(); } else if(e.key === 'Escape') setEditingMessageId(null); };

  const handleOpenReaction = (e: React.MouseEvent, msgId: string) => {
      const rect = e.currentTarget.getBoundingClientRect();
      const distanceToTop = rect.top;
      setPickerPlacement(distanceToTop < 400 ? 'below' : 'above');
  ...
  const toggleReaction = (msgId: string, emoji: string) => {
      ...
      if (hasReacted) removeReaction(msgId, emoji);
  ...
  const getSidebarMembers = () => {
      ...
      const online: typeof allUsers = [], offline: typeof allUsers = [];
    ...
    <div className="flex h-[calc(100vh-4rem)] lg:h-screen overflow-hidden font-sans bg-white dark:bg-[#0B0D0F] relative text-slate-800 dark:text-slate-100">
      ...
      <div className="w-64 bg-gray-50 dark:bg-[#15171B] flex flex-col shrink-0 hidden md:flex border-r border-gray-200 dark:border-white/5">
         <div className="h-16 flex items-center justify-between px-5 border-b border-gray-200 dark:border-white/5">
             <h1 className="font-bold text-gray-900 dark:text-white truncate tracking-tight">Capychat </h1>
         ...
         <div className="flex-1 overflow-y-auto p-3 space-y-6 custom-scrollbar">
             ...
             <div>
                ...
                <div className="space-y-0.5">{publicChannels.map(ch => (
                    <div key={ch.id} onClick={() => joinRoom(ch.name)} className={`group flex items-center justify-between px-3 py-2 rounded-lg cursor-pointer transition-all ${activeRoom === ch.name ? 'bg-blue-50 text-blue-600 dark:bg-blue-600/10 dark:text-blue-400 font-medium' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-white/5'}`}>
                        <div className="flex items-center gap-2.5 overflow-hidden">
                            <Hash size={18} className={activeRoom === ch.name ? 'text-blue-600 dark:text-blue-400' : 'text-gray-400'} />
                            <span className={`text-sm truncate ${ch.unreadCount && ch.unreadCount > 0 ? 'font-bold text-gray-900 dark:text-white' : ''}`}>
                                {ch.name}
                            </span>
                        </div>
                        ...
                        {ch.unreadCount && ch.unreadCount > 0 ? (
                            ...
                            <button onClick={(e) => handleDeleteChannelClick(e, ch.id)} className="opacity-0 group-hover:opacity-100 text-gray-400 hover:text-red-500 p-1"><Trash2 size={12} /></button>
                        )}
                    </div>
                ))}</div>}
             </div>
             ...
             <div>
                ...
                {isLoadingChannels ? <ChannelSkeleton /> : <div className="space-y-0.5">{privateGroups.map(ch => (
                    <div key={ch.id} onClick={() => joinRoom(ch.name)} className={`group flex items-center justify-between px-3 py-2 rounded-lg cursor-pointer transition-all ${activeRoom === ch.name ? 'bg-blue-50 text-blue-600 dark:bg-blue-600/10 dark:text-blue-400 font-medium' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-white/5'}`}>
                        <div className="flex items-center gap-2.5 overflow-hidden">
                            <Lock size={16} className={activeRoom === ch.name ? 'text-blue-600 dark:text-blue-400' : 'text-gray-400'} />
                            <span className={`text-sm truncate ${ch.unreadCount && ch.unreadCount > 0 ? 'font-bold text-gray-900 dark:text-white' : ''}`}>
                                {ch.name}
                            </span>
                        </div>
             ...
             {activeDMs.length > 0 && <div>
                ...
                <div className="space-y-0.5">{activeDMs.map(dm => (
                    <div key={dm.userId} onClick={() => switchToDM(dm)} className={`flex items-center gap-2.5 px-3 py-2 rounded-lg cursor-pointer transition-all ${activeRoom === dm.roomId ? 'bg-blue-50 text-blue-600 dark:bg-blue-600/10 dark:text-blue-400 font-medium' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-white/5'}`}>
      ...
      <div className="flex-1 flex flex-col min-w-0 bg-white dark:bg-[#0B0D0F] relative">
         ...
         {/* MESSAGES AREA */}
         <div 
            className="flex-1 overflow-y-auto custom-scrollbar flex flex-col px-4 pt-4 pb-4 bg-white dark:bg-[#0B0D0F]"
            ref={scrollRef}
                             ...
                             <div className={`group relative flex items-start gap-4 px-2 py-1 hover:bg-gray-50 dark:hover:bg-white/5 -mx-2 rounded-lg transition-colors ${isSequence ? 'mt-0.5' : 'mt-4'}`}>
                                 ...
                                 <div className="flex-1 min-w-0">
                                     ...
                                     {isEditing ? (
                                         <div className="bg-gray-50 dark:bg-[#15171B] p-3 rounded-lg w-full border border-blue-200 dark:border-blue-900/50">
                                             <textarea autoFocus value={editText} onChange={(e) => setEditText(e.target.value)} onKeyDown={handleKeyDownEdit} className="w-full bg-transparent border-none outline-none text-sm resize-none font-sans text-gray-900 dark:text-white" rows={2}/>
                                         ...
                                         <div className="text-[15px] text-gray-800 dark:text-gray-300 leading-relaxed whitespace-pre-wrap font-medium">
                                             ...
                                             {!isEditing && msg.content.match(/(https?:\/\/[^\s]+)/g)?.find(link => link.includes('/assets/')) && <div className="mt-2 max-w-[400px]"><RichLinkPreview url={msg.content.match(/(https?:\/\/[^\s]+)/g)!.find(link => link.includes('/assets/'))!}/></div>}
                                             {msg.attachmentUrl && (
                                                 <div className="mt-2">
                                     ...
                                     <div className="flex items-center gap-2 mt-1">
                                         {msg.reactions && msg.reactions.length > 0 && (
                                             <div className="flex flex-wrap gap-1">
                                                 {Object.entries(reactionCounts || {}).map(([emoji, count]) => {
                                                     const hasMe = msg.reactions?.some(r => r.emoji === emoji && r.userId === user.id);
                                                     return (
                                                         <button 
                                                             key={emoji}
                                                             onClick={() => toggleReaction(msg.id, emoji)}
                                                             className={`flex items-center gap-1 px-1.5 py-0.5 text-xs rounded-md border transition-colors ${hasMe ? 'bg-blue-100 dark:bg-blue-900/30 border-blue-200 dark:border-blue-800' : 'bg-gray-100 dark:bg-white/5 border-transparent hover:border-gray-300 dark:hover:border-white/20'}`}
                                                         >
                                                             <span>{emoji}</span>
                                         ...
                                         {replyCount > 0 && (
                                             <div onClick={() => openThread(msg)} className="flex items-center gap-1 text-xs font-bold text-blue-500 hover:text-blue-400 cursor-pointer select-none">
                                                 ...
                                                 {replyCount} replies
                                                 <span className="text-[10px] text-gray-400 font-normal ml-1">Last reply today at {formatTime(msg.updatedAt)}</span>
                                             </div>
                                         )}
                                     </div>
                                 </div>
                                 ...
                                 {!isEditing && (
                                     <div className="absolute right-4 -top-3 opacity-0 group-hover:opacity-100 bg-white dark:bg-[#1E1F22] shadow-md border border-gray-200 dark:border-white/10 rounded-md flex overflow-hidden transition-opacity z-10">
                                         ...
                                         {isMe && (
                                             <>
                                                 <button onClick={() => handleStartEdit(msg)} className="p-2 text-gray-500 hover:text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/20 transition-colors" title="Edit"><Edit2 size={14} /></button>
                                 ...
                                 {reactingToMessageId === msg.id && (
                                     <div 
                                         ...
                                         className={`absolute right-0 z-50 shadow-2xl rounded-xl border border-gray-200 dark:border-white/10 ${pickerPlacement === 'above' ? 'bottom-full mb-2' : 'top-full mt-2'}`}
                                     >
             ...
             <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 p-4">
                 <div className="w-full max-w-sm bg-white dark:bg-[#1A1D21] border border-gray-200 dark:border-white/10 rounded-xl shadow-2xl p-6 animate-in zoom-in-95">
                     <div className="flex flex-col items-center text-center mb-6">
                         ...
                         <p className="text-sm text-gray-500 dark:text-gray-400">Are you sure you want to delete this channel? All messages will be lost forever.</p>
                     </div>
                     <div className="flex gap-3">
                         <button onClick={() => setChannelToDeleteId(null)} className="flex-1 py-2 bg-gray-100 dark:bg-white/10 text-gray-700 dark:text-white rounded-lg hover:bg-gray-200 dark:hover:bg-white/20">Cancel</button>
                         <button onClick={confirmDeleteChannel} className="flex-1 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 transition-colors">Delete Channel</button>
                     </div>
                 </div>
             </div>
         )}
      ...
      {activeThread ? (
              ...
              <div className="flex-1 overflow-y-auto p-4 custom-scrollbar">
                  {parentMessage && (
                      <div className="mb-6 pb-4 border-b border-gray-200 dark:border-white/10">
                          <div className="flex items-center gap-2 mb-2"><div className="w-8 h-8 rounded-lg bg-gray-200 dark:bg-white/10 flex items-center justify-center font-bold text-sm">{parentMessage.user.avatar ? <img src={parentMessage.user.avatar} className="w-full h-full object-cover rounded-lg"/> : parentMessage.user.name.charAt(0)}</div><span className="font-bold text-sm text-gray-900 dark:text-white">{parentMessage.user.name}</span><span className="text-xs text-gray-500">{formatTime(parentMessage.createdAt)}</span></div>
                          <div className="text-sm text-gray-700 dark:text-gray-300 ml-10">{renderMessageContent(parentMessage.content)}</div>
                      </div>
                  )}
                  {activeThread.replies.length === 0 ? <div className="text-center text-gray-500 text-sm mt-10">No replies yet. Start the conversation!</div> : (activeThread.replies.map(reply => (
          ...
          showMembers && (
              <div className="w-64 bg-white dark:bg-[#15171B] hidden lg:flex flex-col border-l border-gray-200 dark:border-white/5 p-4 overflow-y-auto">
                 {/* ✅ FIXED: Render Avatar in Sidebar */}
